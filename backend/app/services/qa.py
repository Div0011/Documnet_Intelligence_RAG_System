...
SYSTEM_PROMPT = """You are a document Q&A assistant. You will be given a user question and a set of numbered source excerpts pulled from PDFs the user uploaded.
...
def _build_context(chunks):
    ...
    for i, c in enumerate(chunks, start=1):
        meta = c["metadata"]
        lines.append(f"[{i}] (source: {meta['source']}, page {meta['page']})\n{c['text']}")
    return "\n\n".join(lines)
